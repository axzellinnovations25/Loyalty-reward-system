...
const { ok, created, noContent, notFound } = require('../../utils/apiResponse');
...
const getByPhone  = async (req, res, next) => { try { const c = await service.findByPhone(req.shopId, req.params.phone); if (!c) return notFound(res, 'Customer not found'); return ok(res, c); } catch (e) { next(e); } };
...
module.exports = { list, getById, getByPhone, create, update, remove };
